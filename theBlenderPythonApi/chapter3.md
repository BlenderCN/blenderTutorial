...
1.创建支持矩形棱柱以外的选区的ut.act.select_by_loc()函数。
有可能用于圆柱形，球形，二维和一维选择表面。

2.为它们创建额外的ut.create函数和特定于案例的自动命名模式。

3.以与添加ut.act.extrude和ut.act.subdivide相同的方式向ut.act添加其他编辑模式操作。有充分的机会探索并进一步参数化这些函数。
